#ifdef LBmaster_AirdropServer
modded class LB_Airdrop_Base : Container_Base
{
    private string m_UniqueAirdropZoneID; 
    override void EEInit()
    {
        super.EEInit();
        if (GetGame().IsServer() && g_MainConfig && g_MainConfig.EnableAirdropZoneCreation)
        {
            GetGame().GetCallQueue(CALL_CATEGORY_GAMEPLAY).CallLater(CreateAirdropZone, 500, false);
        }
    }

    void CreateAirdropZone()
    {
        if (!GetGame().IsServer() || !g_MainConfig || !g_MainConfig.EnableAirdropZoneCreation)
            return;
        ...
        m_UniqueAirdropZoneID = "AirdropZone_" + center[0].ToString() + "_" + center[2].ToString();

        ZoneBase zone = new ZoneBase();
        zone.name                        = m_UniqueAirdropZoneID;
    ...
    override void EEDelete(EntityAI parent)
    {
        ...
        if (!GetGame().IsServer())
            return;
        ZoneManager zoneManager = ZoneManager.GetInstance();
        if (zoneManager && m_UniqueAirdropZoneID != "")
        {
            bool removed = zoneManager.RemoveDynamicZone(m_UniqueAirdropZoneID);
            if (removed)
                NinjinsPvPPvE.LogDebug("[AirdropZone] Removed zone on deletion: " + m_UniqueAirdropZoneID);
            else
                NinjinsPvPPvE.LogDebug("[AirdropZone] No zone found to remove on deletion: " + m_UniqueAirdropZoneID);
        }
    }
}
#endif
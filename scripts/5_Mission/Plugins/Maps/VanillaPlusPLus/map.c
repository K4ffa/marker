...
modded class VPPMapMenu extends UIScriptedMenu
{
    ...
    override Widget Init()
    {
        ...
        if (!m_MapWidget)
        {
            NinjinsPvPPvE.LogWarning("[VPPMapMenu] ERROR: MapWidget not found.");
            return layoutRoot;
        }
        ...
        if (!canvasLayout)
        {
            NinjinsPvPPvE.LogWarning("[VPPMapMenu] ERROR: Failed to load canvas layout.");
            return layoutRoot;
        }
        m_DrawCanvas = CanvasWidget.Cast(canvasLayout.FindAnyWidget("ninjindrawCanvas"));
        if (!m_DrawCanvas)
        {
            NinjinsPvPPvE.LogWarning("[VPPMapMenu] ERROR: drawCanvas not found in layout.");
            return layoutRoot;
        }
        m_MapDrawer = MapDrawer.GetInstance(m_MapWidget, m_DrawCanvas);
        if (!m_MapDrawer)
        {
            NinjinsPvPPvE.LogWarning("[VPPMapMenu] ERROR: Failed to initialize MapDrawer.");
        }
        return layoutRoot;
    }

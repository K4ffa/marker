modded class MapMenu : UIScriptedMenu
{
    ...
    override Widget Init()
    {
        ...
        if (!m_MapWidget)
        {
            NinjinsPvPPvE.LogWarning("[MapMenu] ERROR: MapWidget not found.");
            return layoutRoot;
        }
        Widget canvasLayout = GetGame().GetWorkspace().CreateWidgets("NinjinsPvPPvE/gui/layouts/NinjinsMapCanvasOnly.layout", m_MapWidget);
        if (!canvasLayout)
        {
            NinjinsPvPPvE.LogWarning("[MapMenu] ERROR: Failed to load canvas-only layout.");
            return layoutRoot;
        }
        m_DrawCanvas = CanvasWidget.Cast(canvasLayout.FindAnyWidget("ninjindrawCanvas"));
        if (!m_DrawCanvas)
        {
            NinjinsPvPPvE.LogWarning("[MapMenu] ERROR: drawCanvas not found in layout.");
            return layoutRoot;
        }
        m_MapDrawer = MapDrawer.GetInstance(m_MapWidget, m_DrawCanvas);
        if (!m_MapDrawer)
        {
            NinjinsPvPPvE.LogWarning("[MapMenu] ERROR: Failed to initialize MapDrawer.");
        }
        return layoutRoot;
    }
